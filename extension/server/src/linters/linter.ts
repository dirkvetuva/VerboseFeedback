...
export class Linter {
    ...
    protected constructor(name: string, settings: Settings, workspaceRoot: string, requireConfig: boolean) {
        ...
        this.enabled = true;
        this.active = true;
        this.language = settings['verbosefeedback'].language;
        this.standard = settings['verbosefeedback'].standard;
        this.defines = settings['verbosefeedback'].defines;
        this.undefines = settings['verbosefeedback'].undefines;
        this.includePaths = settings['verbosefeedback'].includePaths;
    }

    protected cascadeCommonSettings(key: string) {
        let checkKey = (item: string): boolean => {
            return this.settings['verbosefeedback'][key as keyof Settings['verbosefeedback']].hasOwnProperty(item) &&
                this.settings['verbosefeedback'][key as keyof Settings['verbosefeedback']].hasOwnProperty(item) !== null &&
                (this.settings['verbosefeedback'][key as keyof Settings['verbosefeedback']] as any)[item] !== null;
        };

        let maybe = (orig: string[] | string, maybeKey: string) => {
            if (checkKey(maybeKey)) {
                if (_.isArray(orig)) {
                    return (this.settings['verbosefeedback'][key as keyof Settings['verbosefeedback']] as any)[maybeKey];
        ...
        return new Promise((resolve, reject) => {
            ...
            which(this.executable, { path: paths.toString() }, (err: any, result: any) => {
                if (err) {
                    ...
                    if (this.settings['verbosefeedback'].debug) {
        ...
        lines.forEach(line => {
            ...
            if (parsed) {
                ...
                if (parsed.parseError) {
                    if (this.settings['verbosefeedback'].ignoreParseErrors) {
