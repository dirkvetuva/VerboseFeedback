...
namespace CommandIds {
    export const analyzeActiveDocument: string = 'verbosefeedback.analyzeActiveDocument';
    export const analyzeWorkspace: string = 'verbosefeedback.analyzeWorkspace';
}
...
connection.onDidChangeConfiguration(change => {
    if (hasConfigurationCapability) {
        ...
        globalSettings = <Settings>(change.settings['verbosefeedback'] || defaultSettings);
    }
...
//
connection.onNotification('returnFeedback', (_params: any) => {
...
connection.onNotification('onBuild', async (params: any) => {
    ...
    const userLintOn: Lint = toLint(settings['verbosefeedback'].run);
...
async function reconfigureExtension(currentSettings: Settings, workspaceRoot: string): Promise<Linter[]> {
    ...
    if (currentSettings['verbosefeedback'].clang.enable) { linters.push(await (new Clang(currentSettings, workspaceRoot).initialize()) as Clang); }
                        ...
                        try {
                            ...
                            _.each(globbed_path, (gpath: string | EntryItem) => {
                                ...
                                if (path.normalize(currentFilePath).startsWith(path.normalize(workspaceRoot!))) {
                                    ...
                                    _.each(currentSettings['verbosefeedback'].excludeFromWorkspacePaths, (excludedPath: string) => {
...
async function getActiveConfigurationName(currentSettings: Settings): Promise<string> {
    ...
    return RobustPromises.retry(40, 250, 1000, () => connection.sendRequest<string>('verbosefeedback.cpptools.activeConfigName')).then(r => {
...
function makeDiagnostic(documentLines: string[] | null, msg: InternalDiagnostic, uri: string): Diagnostic {
    let line: number;
    ...
    if (msg.message) {
        let all_err_type: ErrLib[] = diag_lib['errors'];
