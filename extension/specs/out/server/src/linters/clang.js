...
class Clang extends linter_1.Linter {
    ...
    buildCommandLine(fileName, tmpFileName) {
        ...
        let blocks = this.settings['verbosefeedback'].clang.blocks ?
            ['-fblocks'] : [];
        let includeArgParams = this.expandedArgsFor('-include', false, this.settings['verbosefeedback'].clang.includes, null);
        let warningsParams = this.expandedArgsFor('-W', true, this.settings['verbosefeedback'].clang.warnings, null);
        let standardParams = this.expandedArgsFor('--std=', true, this.standard, ['c11', 'c++11']);
        let standardLibParams = this.expandedArgsFor('--stdlib=', true, this.settings['verbosefeedback'].clang.standardLibs, null);
    ...
    getSeverityCode(severity) {
        let output = this.settings['verbosefeedback'].clang.severityLevels[severity];
